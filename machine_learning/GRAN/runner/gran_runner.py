...
class GranRunner(object):
  ...
  def __init__(self, config):
    ...
    self.debug_dir = os.path.join(self.config.save_dir, "debug_iter")
      ...
      for inner_iter in range(len(train_loader) // self.num_gpus):
        ...
        if self.use_gpu:
          for _ in self.gpus:
            ...
            iter_count += 1
        # ────────────────────────────────────────────────────────────────
        # DEBUG DUMP EVERY 100 ITERATIONS
        # uses the first GPU’s sub‑batch and the first forward‑pass
        if iter_count % 100 == 0:
            dump_dir = self.debug_dir
            
            sample   = batch_data[0][0]                
            gid      = sample["graph_id"].item()       

            # input γγ image
            yy_img = sample["yy_matrix"][0, 0].cpu().numpy()
            plt.imsave(os.path.join(dump_dir,
                     f"iter_{iter_count:07d}_input.png"),
                     yy_img, cmap="gray", vmin=0, vmax=1)

            # sub‑graph used for loss
            N_sub   = sample["num_nodes_gt"][0].item()
            adj_sub = sample["adj"][0, 0, :N_sub, :N_sub].cpu().numpy()
            G_sub   = nx.from_numpy_array(adj_sub)

            # full level‑scheme
            item = self.graphs_train[gid]
            full_G = item[0] if isinstance(item, (tuple, list)) else item

            with open(os.path.join(dump_dir,
                       f"iter_{iter_count:07d}_graphs.txt"), "w") as f:
                f.write("SUB‑GRAPH (loss)\n")
                f.write(str(sorted(G_sub.edges())) + "\n\n")
                f.write("FULL LEVEL‑SCHEME (npz)\n")
                f.write(str(sorted(full_G.edges())) + "\n")
