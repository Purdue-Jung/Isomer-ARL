...
# Randomly split levels into exactly 8 bands
...
def plot_multiple_bands(bands: List[BandFromLevels], title: str = "Level Schemes"):
    ...
    ax.set_title(title)
    ax.set_ylim(0, max(max(b.nodes.values()) for b in bands if b.nodes) + y_margin)
...
if __name__ == "__main__":
    ...
    if not args.csv_file or not os.path.isfile(args.csv_file):
        ...
        try:
            ...
            bands = split_into_bands(levels)  # always 8 bands
            plot_multiple_bands(bands, title="8-Band Level Scheme")
