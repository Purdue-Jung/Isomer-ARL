...
class ELevel(CsvColumn):
    ...
    def get_tokens(self):
        ...
        if len(tokens) < 2:
            raise ValueError(f"ELevel string \"{self.value}\" split in fewer than 2 tokens")
...
class JPi(CsvColumn):
    ...
    def get_tokens(self):
        ...
        if not tokens:
            raise ValueError(f"JPi string \"{self.value}\" split into fewer than 1 token")
        ...
        if "," in jpi:
            raise ValueError(f"JPi string \"{self.value}\" contains a comma")

        if "+" not in jpi and "-" not in jpi:
            raise ValueError(f"JPi string \"{self.value}\" does not contain '+' or '-'")
...
class Converter:
    ...
    #########################
    def _open_csv(self): 
        self._csv_file = open(self.csv_filename, 'r', encoding='utf-8-sig')

    #########################
    def _open_ags(self):
        self._ags_file = open(self.ags_filename, 'w', encoding='utf-8')
        self._ags_file.writelines ( [
    ...
    def _set_column_indexes(self, headers):
        ...
        if self.debug:
            print(f"\n{self.csv_filename}:{self.csv_line_number}")
            for name, col in self.columns.items():
                print(f"\t{name} is column {col.index}")
